...
public class ReportResourceDeploymentStatusWrapper extends ResourceMessageWrapper implements ReportResourceDeploymentStatus  {

  private static Logger log = LoggingUtils.getLogger(ReportResourceDeploymentStatusWrapper.class);
    

    public ReportResourceDeploymentStatusWrapper(Element internal) {
    super(internal);
    ...
    public ReportResourceDeploymentStatusWrapper(Factory factory, QName qname) {
      super(factory, qname);
      // TODO Auto-generated constructor stub
    }
    ...
    public void constructMessage (ReportResourceDeploymentStatusWrapper msg,String messageSource,String resourceid,String status,
        String oid,String pid,String incident,String senderFn,String senderLn) { 
      msg.setMessageID(ResourceMessageIdFactory.getNextId(messageSource));
      msg.setSentDateTime(ERENDatatypeFactory.factory.newXMLGregorianCalendar(new GregorianCalendar()));
      msg.setMessageContentType("ReportResourceDeploymentStatus");
      msg.setOriginatingMessageID(oid);
      msg.setPrecedingMessageID(pid);
      IncidentInformationType ii = msg.addIncidentInformation();
      ii.setIncidentDescription(incident);
      ContactInformationType contact = msg.addContactInformation();
      contact.setContactRole(ContactRoleType.SENDER);
      PartyType aci = contact.addAdditionalContactInformation();
      PartyNameType name = aci.addPartyName();
      PersonName person = name.addPersonName();
      person.setFirstName(senderFn);
    }
    ...
    public boolean isResourceReady (String resourceID) { 
      return isResourceStatus(resourceID,DeploymentStatusType.READY);
    }
    
    public boolean isResourceInUse (String resourceID) { 
      return isResourceStatus(resourceID,DeploymentStatusType.IN_USE);
    }
    
    public boolean isResourceNonFunctional (String resourceID) { 
      return isResourceStatus(resourceID,DeploymentStatusType.NON_FUNCTIONAL);
    }
    
    public boolean isResourceReleased (String resourceID) { 
      return isResourceStatus(resourceID,DeploymentStatusType.RELEASED);
    }
    
    private boolean isResourceTypeStatus (String type, String status) { 
      for (ResourceInformation ri : getResourceInformation()) {
        Resource r = ri.getResource();
        if (r.getTypeStructure().getValue().equals(type)) { 
    ...
    private boolean isResourceStatus (String resourceID,String status) { 
      for (ResourceInformation ri : getResourceInformation()) {
        Resource r = ri.getResource();
    ...
    private boolean isResourceStatus (String resourceID,DeploymentStatusType status) { 
      for (ResourceInformation ri : getResourceInformation()) {
        Resource r = ri.getResource();
        if (r.getResourceID().equals(resourceID)) { 
          for (String s : r.getResourceStatus().getDeploymentStatus().getValue()) { 
